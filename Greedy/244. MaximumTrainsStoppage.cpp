...
int maxStop(int arr[][3])
{
	...
	// as per platform number and making departure time first element of pair
	for (int i = 0; i < m; i++)
		vect[arr[i][2]].push_back(
		    make_pair(arr[i][1], arr[i][0]));

	// sort trains for each platform acc. to dept. time
	for (int i = 0; i <= n; i++)
	...
	for (int i = 0; i <= n; i++) {
		...
		// first train for each platform will be selected
		int x = 0;
		count++;
